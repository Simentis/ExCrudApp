...
export class BookPostService {
  ...
  myApiUrl: string;
  httpOptions = {
    headers: new HttpHeadersimport { Injectable } from '@angular/core';
    import { HttpClient, HttpHeaders } from '@angular/common/http';
    import { Observable, throwError } from 'rxjs';
    import { retry, catchError } from 'rxjs/operators';
    import { environment } from 'src/environments/environment';
    ...
    export class BookPostService {
      ...
      saveBookPost(bookPost): Observable<BookPost> {
      ...
      updateBookPost(postId: number, bookPost): Observable<BookPost> {
  ...
  errorHandler(error: { error: { message: string; }; status; message; }) {
    let errorMessage = '';
    if (error.error instanceof ErrorEvent) {
      // Get client-side error
      errorMessage = error.error.message;
